                ...
                {problem.sample_test_cases.map((tc, tci) => (
                  <div key={tci} className="rounded-lg border border-slate-200 bg-slate-50 p-4">
                    <div className="mb-2 flex items-center justify-between">
                      ...
                      <button
                        onClick={() => removeTestCase('sample', tci)}
                ...
                {problem.hidden_test_cases.map((tc, tci) => (
                  <div key={tci} className="rounded-lg border border-slate-200 bg-slate-50 p-4">
                    <div className="mb-2 flex items-center justify-between">
                      ...
                      <button
                        onClick={() => removeTestCase('hidden', tci)}
                        className="text-red-400 hover:text-red-600"
                      >
                        <Trash2 className="h-3 w-3" />
                      </button>
...
export default function AdminAssessmentForm() {
  ...
  useEffect(() => {
    if (isEdit) {
      apiFetch(`/api/programming-assessment/admin/assessments/${assessmentId}`).then((data) => {
        setAssessment({
  ...
  async function handleSave() {
    ...
    try {
      let id = assessmentId;
      if (!isEdit) {
        const created = await apiFetch('/api/programming-assessment/admin/assessments', {
          method: 'POST',
          body: JSON.stringify(assessment),
        });
        id = created.assessment.id;
      } else {
        await apiFetch(`/api/programming-assessment/admin/assessments/${id}`, {
          method: 'PATCH',
          body: JSON.stringify(assessment),
        });
      }
      await apiFetch(`/api/programming-assessment/admin/assessments/${id}/problems`, {
        method: 'PUT',
        body: JSON.stringify({ problems }),
      });
      navigate('/admin/programming/assessments');
    } catch (err) {
  ...
  return (
    <div className="page-stack max-w-5xl">
      <button
        onClick={() => navigate('/admin/programming/assessments')}
