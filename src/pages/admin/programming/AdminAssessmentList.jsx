...
export default function AdminAssessmentList() {
  ...
  useEffect(() => {
    apiFetch('/api/programming-assessment/admin/assessments').then(setData);
  }, []);

  async function handleToggleStatus(assessment) {
    const newStatus = assessment.status === 'published' ? 'draft' : 'published';
    await apiFetch(`/api/programming-assessment/admin/assessments/${assessment.id}/status`, {
      method: 'PATCH',
  ...
  async function handleDelete(id) {
    ...
    await apiFetch(`/api/programming-assessment/admin/assessments/${id}`, { method: 'DELETE' });
    setData((prev) => ({
      ...prev,
    ...
    <section className="page-stack">
      <div className="page-hero flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <h2 className="mt-2 text-3xl font-black text-slate-900">Programming Assessments</h2>
          <p className="mt-2 text-sm leading-6 text-slate-500">
            Create and manage multi-problem coding tests for your students.
          </p>
        </div>
        <Link to="/admin/programming/assessments/create" className="btn-primary">
      ...
      {data.assessments.length === 0 ? (
        <div className="surface p-12 text-center">
          ...
          <Link to="/admin/programming/assessments/create" className="mt-2 inline-block text-sm font-semibold text-emerald-600 hover:text-emerald-700">
                  ...
                  <tr key={a.id} className="hover:bg-slate-50/70">
                    ...
                    <td>
                      <div className="flex items-center justify-end gap-2">
                        <button
                          onClick={() => navigate(`/admin/programming/assessments/${a.id}/problems`)}
                        ...
                        <button
                          onClick={() => navigate(`/admin/programming/assessments/${a.id}/results`)}
                          className="rounded-md border border-slate-200 bg-white p-2 hover:bg-slate-50"
                          title="View Results"
                        >
