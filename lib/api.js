...
export async function submitAnswer(
  ...
  // Derive filename from mimeType
  const audioMime = audioBlob.type || "audio/webm";
  const audioExt  = audioMime.includes("ogg") ? "ogg" : audioMime.includes("mp4") ? "mp4" : "webm";

  if (videoBlob) {
    // Backend has /api/answer_video_with_audio — separate audio + video
    const videoMime = videoBlob.type || "video/webm";
    ...
    if (!res.ok) {
      const text = await res.text();
      console.error("🔴 Backend 500 detail:", text);
      throw new Error(text || `Server error ${res.status}`);
    }
    return res.json();
  } else {
    // Audio-only fallback → /api/answer_video (backend handles audio-only webm)
